...
const SlideAside = ({ activeItem }) => {
  return (
    <div className="app-slide-aside w-full md:max-w-xl relative z-20">
      ...
      <div className="
        ...
        group min-h-[300px] flex flex-col justify-center
      ">
        {/* Internal Glow Effect */}
        <div 
           className="absolute -top-20 -right-20 w-64 h-64 rounded-full blur-[90px] opacity-30 transition-colors duration-1000 pointer-events-none"
           style={{ backgroundColor: activeItem.color }}
        />

        {/* Brand Name */}
        <h3 className="text-white/40 font-mono text-sm tracking-widest mb-2 uppercase">
        ...
        <div key={activeItem.id} className="animate-content-enter relative z-10">
          ...
          {/* Title */}
          <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter mb-4 leading-tight drop-shadow-xl">
            {activeItem.name}
          </h1>

          {/* Description */}
          <p className="text-white/80 text-sm md:text-base leading-relaxed font-light max-w-md drop-shadow-md">
            {activeItem.desc}
          </p>
...
const SlideParams = ({ activeItem }) => {
  return (
    <div className="
      absolute bottom-10 left-0 right-0 z-30
      ...
      ">
        <ul className="flex flex-wrap justify-center gap-x-6 gap-y-3 md:gap-12">
          {[
            ...
            <li key={index} className="text-center group flex flex-col items-center min-w-[60px]">
              <div className="flex items-baseline justify-center gap-1">
                <span className="text-sm text-white/40 font-light">+</span>
                <AnimateValue 
                  className="text-xl md:text-2xl font-bold text-white group-hover:text-cyan-400 transition-colors" 
                  value={item.stat.value} 
                  delay={item.delay} 
                />
                <span className="text-[10px] font-mono text-white/60">{item.stat.suffix}</span>
              </div>
              <p className="text-[9px] uppercase tracking-[0.2em] text-white/40 mt-1">{item.stat.label}</p>
            </li>
          ))}
        </ul>
      </div>
...
const Slide = ({ activeSlide, animationForward, setAnimationState, ANIMATION_PHASES }) => {
  return (
    <div className={`app-slide flex flex-col items-center justify-center w-full h-full relative ${animationForward ? "animation-forward" : "animation-back"}`}>
      ...
      {/* Adjusted translate-y to bring it down slightly (from -16 to -4) */}
...
const SliderNavigation = ({ activeSlide, setActiveSlide, itemsNames }) => (
  <div className="app-slider-navigation absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col gap-4">
    {itemsNames.map((item, index) => {
      ...
      return (
        <button
          ...
          className={`
            group relative w-64 p-5 text-left rounded-xl border backdrop-blur-lg transition-all duration-500 ease-out overflow-hidden
            ${isActive 
              ? 'bg-white/10 border-white/30 shadow-[0_0_30px_rgba(0,0,0,0.5)] -translate-x-4 scale-105' 
              : 'bg-black/40 border-white/5 hover:bg-white/5 hover:border-white/20 translate-x-0'
            }
          `}
        >
          {/* Active Glow Bar */}
          <div 
            className={`absolute left-0 top-0 bottom-0 w-1 rounded-l-xl transition-all duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`}
            style={{ backgroundColor: item.color }}
          />
          
          {/* Ambient Background Fill */}
          <div 
            className={`absolute inset-0 transition-opacity duration-500 pointer-events-none ${isActive ? 'opacity-20' : 'opacity-0 group-hover:opacity-5'}`}
            style={{ 
...
const Header = () => (
  <div className="app-header flex justify-between items-center py-6 md:py-8 relative z-50">
    <div className="app-header__logo">
       <h1 className="text-3xl md:text-5xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-white/10 drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]">
         ZERONE
         <span className="text-cyan-400 font-mono text-xl md:text-2xl ml-2 tracking-widest font-light">2026</span>
       </h1>
    </div>
...
class TechnicalPage extends Component {
  ...
  render() {
    return (
      <div className="technical-page-wrapper fixed inset-0 bg-black text-white overflow-hidden font-sans">
        ...
        {/* Content Container */}
        <div className="container mx-auto px-6 h-full flex flex-col relative z-10 pointer-events-none">
          {/* Header */}
